#!/usr/bin/env python3
"""
  This script makes available functionality related to "each_n_days".
...
class StaticEachNDays:
  ...
  @classmethod
  def write_to_dictfile_each_n_days_from_dictlist(cls, filename=None, dictlist=None):
  ...
  @staticmethod
  def write_to_jsonfile_each_n_days_from_dictlist(filename=None, dictlist=None):
    if filename is None:
      filename = EACH_N_DAYS_JSON_FILENAME
    if dictlist is None or len(dictlist) == 0:
      return False
  ...
  @classmethod
  def write_to_db_each_n_days_from_dictlist(cls, dictlist=None):
    if dictlist is None or len(dictlist) == 0:
      return False
    n_updated_rows = cls.dbupdate_each_n_days_for_dld_from_dictlist(dictlist)
    if n_updated_rows > 0:
      return True
    return False
  ...
  def show_dowloadables_at_moment():
    ...
    for i, ytchannel in enumerate(ytchannels):
      ...
      print(i+1, is_downloadable, nextdate, '<=', ytchannel.each_n_days_for_dld, '+', ytchannel.scrapedate,
            ytchannel.nname)
      if is_downloadable:
        n_of_dlds += 1
...
class EachNDaysForDownloadSyncer:
  ...
  def get_values_for_from_means(self):
    values_for_from_means = [
      self.EACH_N_DAYS_ORIGIN_DICTFILE,
      self.EACH_N_DAYS_ORIGIN_JSONFILE,
      self.EACH_N_DAYS_ORIGIN_DB,
    ]
    return values_for_from_means

  def get_values_for_to_means(self):
    values_for_to_means = [
      self.EACH_N_DAYS_TARGET_DICTFILE,
      self.EACH_N_DAYS_TARGET_JSONFILE,
      self.EACH_N_DAYS_TARGET_DB,
    ]
  ...
  def check_same_means_from_to_if_so_raise(self):
    ...
    if self.from_means == self.EACH_N_DAYS_ORIGIN_DICTFILE and \
      ...
      error_msg = 'Equal means from/to =  %s and %s' % (self.from_means, self.to_means)
            ...
            and self.to_means == self.EACH_N_DAYS_TARGET_JSONFILE:
      error_msg = 'Equal means from/to =  %s and %s' % (self.from_means, self.to_means)
    elif self.from_means == self.EACH_N_DAYS_ORIGIN_DB \
            and self.to_means == self.EACH_N_DAYS_TARGET_DB:
      error_msg = 'Equal means from/to =  %s and %s' % (self.from_means, self.to_means)
