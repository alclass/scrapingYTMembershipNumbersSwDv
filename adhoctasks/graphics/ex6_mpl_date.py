...
Show how to make infodate plots in Matplotlib using infodate tick locators and
formatters.  See :doc:`/gallery/ticks_and_spines/major_minor_demo` for more
information on controlling major and minor ticks.

All matplotlib infodate plotting is done by converting infodate instances into days
...
# Load a numpy structured array from yahoo csv data with fields infodate, open,
# close, volume, adj_close from the mpl-data/example directory.  This array
# stores the infodate as an np.datetime64 with a day unit ('D') in the 'infodate'
# column.
with cbook.get_sample_data('goog.npz') as datafile:
    data = np.load(datafile)['price_data']

fig, ax = plt.subplots()
ax.plot('infodate', 'adj_close', data=data)
...
datemin = np.datetime64(data['infodate'][0], 'Y')
datemax = np.datetime64(data['infodate'][-1], 'Y') + np.timedelta64(1, 'Y')
